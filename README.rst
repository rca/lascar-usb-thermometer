...
`Ángel Sancho`_'s lascar-usb-termometer, which can be found at:
...
example, sending control commands to usb device via `hid_get_input_report`_.
...
return 10 readings and then exit.  Setting count to 0 will collect
data indefinitely.
